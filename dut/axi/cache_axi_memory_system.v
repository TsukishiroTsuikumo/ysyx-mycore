...
`ifndef SYNTHESIS
    ...
    initial begin
        if ((ADDR_WIDTH != 32) || (DATA_WIDTH != 32) || (ID_WIDTH != 2)) begin
            $error("cache_axi_memory_system: validated configuration is 32-bit AXI4 with 2-bit IDs");
        end
        if (LINE_BYTES != 16) begin
            $error("cache_axi_memory_system: validated cache line size is 16 bytes");
        end
    end
`endif
