class instr_base_test extends uvm_test;
    `uvm_component_utils(instr_base_test)
    ...
    function new(string name = "instr_base_test", uvm_component parent);
        super.new(name, parent);
    endfunction

    virtual function void build_phase(uvm_phase phase);
        super.build_phase(phase);
        env = instr_test_env::type_id::create("env", this);
        if(!uvm_config_db#(virtual icache_if)::get(this, "", "vif", ic_vif)) begin
            `uvm_fatal("INSTR_BASE_TEST", "Failed to get icache interface")
        end
        if(!uvm_config_db#(virtual probe_if)::get(this, "", "probe", probe_vif)) begin
            `uvm_fatal("INSTR_BASE_TEST", "Failed to get probe interface")
        end
        if(!uvm_config_db#(virtual dcache_if)::get(this, "", "vif", dc_vif)) begin
            `uvm_fatal("INSTR_BASE_TEST", "Failed to get dcache interface")
        end
    endfunction
