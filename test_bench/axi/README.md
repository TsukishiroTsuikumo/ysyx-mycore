...
the current core testbench. `mycore_system` exports an explicit `mon_axi_*`
observation interface at these boundaries:

- DCache write channels: `dc_axi_*`
- post-arbitration I/D read channels: `arb_axi_*`

`mycore_wrapper` maps those flat ports to `shared_axi_if_inst`; no internal DUT
hierarchy is sampled. The verification adapter ties REGION/USER to zero and
derives the verification-only owner field from the fixed ICache/DCache AXI IDs.
